...
#'
calcFishNativeMets <- function(
    indata, sampID = "UID", dist = "IS_DISTINCT",
  ...
  totals <- aggregate(
    x = list(TOTLNIND = inCts$TOTAL, TOTLNTAX = inCts$IS_DISTINCT), by = inCts[c("SAMPID")],
    FUN = sum
  )
  ...
  outMet.1 <- with(inCts.1, aggregate(
    x = list(ALIENNTAX = alienntax, NAT_TOTLNTAX = natntax, NAT_TOTLNIND = natnind),
    by = inCts.1[c("SAMPID")], FUN = function(x) {
      sum(x, na.rm = T)
    }
  ))
  outMet.2 <- with(inCts.1, aggregate(
    x = list(ALIENPIND = alienpind, ALIENPTAX = alienptax, NAT_PIND = natpind, NAT_PTAX = natptax),
    by = inCts.1[c("SAMPID")], FUN = function(x) {
      round(sum(x, na.rm = T) * 100, 2)
    }
  ))
