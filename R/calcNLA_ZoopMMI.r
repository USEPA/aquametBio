...
calcNLA_ZoopMMI <- function(inMets, sampID = "UID", ecoreg = "ECOREG", totlnind = "TOTL_NIND") {
  ...
  cfVal <- data.frame(
    ...
    PARAMETER = c(
      "FINE_BIO", "SIDID_PIND", "DOM1_300_COPE_PBIO", "FAM300_NAT_NTAX",
      "COLLO_PBIO", "OMNI_PTAX",
    ...
    CEILING = c(
      173.28, 24.88, 100, 15, 5.64, 47.06,
  ...
  matchMets <- reshape(inMets,
    idvar = c("SAMPID", "ECO_BIO", "TOTLNIND"),
    direction = "long",
  ...
  if (nrow(numMets) > 0) {
    return(print(paste0(
      "Missing metrics values for these samples: ",
      numMets$SAMPID,
  ...
  condTholds <- data.frame(
    ECO_BIO = c("EHIGH", "CPL", "WMTNS", "PLAINS", "UMW"),
  ...
  mmiOut.final <- subset(mmiOut.final, select = c(
    sampID, "SAMPID", "ECO_BIO", "MMI_ZOOP",
    "ZOOP_MMI_COND",
    names(mmiOut)[names(mmiOut) %nin% c(
      sampID, "SAMPID",
      "ECO_BIO",
      "MMI_ZOOP",
      "ZOOP_MMI_COND"
