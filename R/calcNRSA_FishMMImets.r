...
#' @keywords survey
calcNRSA_FishMMImets <- function(
    indata, inTaxa = NULL, sampID = "UID", ecoreg = NULL,
  ...
  necTraits <- c(
    taxa_id, tol, vel, habitat, trophic, migr,
    reprod, family, genus, comname
  )
  if (any(necTraits %nin% names(inTaxa))) {
    msgTraits <- which(necTraits %nin% names(inTaxa))
    print(paste(
      "Some of the traits are missing from the taxa list. The following are required for metric calculations to run:",
      necTraits[msgTraits]
    ))
    return(NULL)
  }
  ...
  totals <- aggregate(
    x = list(TOTLNIND = indata.2$TOTAL, TOTLNTAX = indata.2$IS_DISTINCT), by = indata.2[c("SAMPID")],
    FUN = sum
  )
  ...
  outNat.1 <- aggregate(
    x = list(ALIENNTAX = inNat$ALIENNTAX, NAT_TOTLNTAX = inNat$NAT_NTAX), by = inNat[c("SAMPID")],
    FUN = function(x) {
      sum(x, na.rm = T)
    }
  )
  ...
  outNat.2 <- aggregate(
    x = list(ALIENPIND = inNat$ALIENPIND, NAT_PIND = inNat$NAT_PIND, NAT_PTAX = inNat$NAT_PTAX),
    by = inNat[c("SAMPID")], FUN = function(x) {
  ...
  outLong <- reshape(outAll,
    idvar = c(sampID, ecoreg, "SAMPID"), direction = "long",
