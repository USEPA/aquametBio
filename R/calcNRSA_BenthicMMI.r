...
calcNRSA_BenthicMMI <- function(inMets, sampID = "UID", ecoreg = "ECOREG", totlnind = "TOTLNIND") {
  ...
  matchMets <- reshape(inMets,
    idvar = c("SAMPID", "ECO9", "TOTLNIND"), direction = "long",
  ...
  cfVal <- data.frame(metnames,
    DISTRESP = c(
      "NEGATIVE", rep("POSITIVE", 4), "NEGATIVE", "POSITIVE", "NEGATIVE", rep("POSITIVE", 7),
    ...
    CEILING = c(
      73.0, 3.31, 9, 54.8, 17, 50, 57.6, 76.2, 12, 70, 24, 86.1, 50, 3.07, 6, 35.3, 7, 28, 28.6, 3.44, 12, 25, 25, 27.6,
  ...
  scored.mets$PARAMETER[scored.mets$PARAMETER %in% c("BURRPTAX", "CLNGPTAX", "CLNGNTAX")] <- "HABT_PT"
  ...
  cond.mmi$ECO9 <- as.character(cond.mmi$ECO9)
  ...
  ww <- rbind(
    subset(scored.mets, select = c("SAMPID", "ECO9", "PARAMETER", "RESULT")),
  ...
  mmiOut.final <- subset(mmiOut.final, select = c(
    sampID, "SAMPID", "ECO9", "MMI_BENT", "BENT_MMI_COND",
