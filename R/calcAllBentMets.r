...
#' bentMetrics <- calcAllBentMets(
#'   indf = bentEx, inTaxa = bentTaxa_nrsa,
...
calcAllBentMets <- function(indf, inTaxa, sampID = "UID", dist = "IS_DISTINCT",
  ...
  tax.1 <- reshape(taxMet,
    idvar = sampID, direction = "long", varying = c(
      "AMPHNTAX", "AMPHPIND",
    ...
    v.names = "value", timevar = "variable", times = c(
      "AMPHNTAX", "AMPHPIND",
      "AMPHPTAX", "CHIRNTAX", "CHIRPIND",
      "CHIRPTAX", "CRUSNTAX", "CRUSPIND", "CRUSPTAX",
      "DIPTNTAX", "DIPTPIND", "DIPTPTAX", "EPHENTAX",
