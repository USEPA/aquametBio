...
calcZoopDivMetrics <- function(indata, sampID, is_distinct,
  ...
  hprime <- aggregate(
    x = list(HPRIME_NIND = calcData[, "prop.cnt"]),
    by = calcData[sampID],
    FUN = function(x) {
      round(-1 * sum(x * log(x), na.rm = T), 4)
    }
  )

  simpson <- aggregate(
    x = list(SIMPSON_NIND = calcData[, "prop.cnt"]),
    by = calcData[sampID],
    FUN = function(x) {
      round(sum(x * x, na.rm = T), 4)
    }
  )
  ...
  pie <- aggregate(
    x = list(PIE_NIND = pie.in[, "CALCPROP"]),
    by = pie.in[sampID],
    FUN = function(x) {
  ...
  if (!is.null(density)) {
    hprime.den <- aggregate(
      x = list(HPRIME_DEN = calcData[, "prop.den"]),
      by = calcData[sampID],
      FUN = function(x) {
        round(-1 * sum(x * log(x), na.rm = T), 4)
      }
    ...
    simpson.den <- aggregate(
      x = list(SIMPSON_DEN = calcData[, "prop.den"]),
      by = calcData[sampID],
      FUN = function(x) {
        round(sum(x * x, na.rm = T), 4)
      }
  ...
  if (!is.null(biomass)) {
    hprime.bio <- aggregate(
      x = list(HPRIME_BIO = calcData[, "prop.bio"]),
      by = calcData[sampID],
      FUN = function(x) {
        round(-1 * sum(x * log(x), na.rm = T), 4)
      }
    ...
    simpson.bio <- aggregate(
      x = list(SIMPSON_BIO = calcData[, "prop.bio"]),
      by = calcData[sampID],
      FUN = function(x) {
        round(sum(x * x, na.rm = T), 4)
      }
