...
calcFishTrophicMets <- function(
  ...
  if (any(optTraits %nin% names(inTaxa))) {
    ...
    print(paste(
      "Optional trait is missing from the taxa list. Any tolerance metrics also using this trait will not be calculated:\n",
      optTraits[msgTraits], "\n"
    ))
  }
