...
calcFishMMI <- function(inMets, sampID = "UID", ecoreg = "ECOREG", lwsarea = "LWSAREA") {
  ...
  metnames <- data.frame(
    ECO9 = c(
      rep("CPL", 8), rep("NAP", 8), rep("NPL", 8), rep("SAP", 8), rep("SPL", 8),
      rep("TPL", 8), rep("UMW", 8), rep("WMT", 8), rep("XER", 8)
    ),
    PARAMETER = c(
      c(
        "ALIENPIND", "RBCATONTAX", "LOTPIND", "INTLMIGRPTAX", "LITHPIND", "NAT_TOTLNTAX", "TOLRNTAX",
        "INVPTAX"
      ),
      c(
        "ALIENNTAX", "SALMNTAX", "NAT_RHEOPIND", "INTLMIGRPIND", "LITHPTAX", "NTOLPTAX", "TOLRNTAX",
        "INVNTAX"
      ),
      c(
        "ALIENNTAX", "NAT_CYPRPIND", "LOTNTAX", "MIGRNTAX", "LITHPIND", "NTOLPTAX", "NAT_INTLPIND",
        "NAT_CARNNTAX"
      ),
      c(
        "NAT_PTAX", "NAT_CENTNTAX", "NAT_NTOLBENTPTAX", "NAT_MIGRNTAX", "NAT_LITHPIND", "NTOLPTAX",
        "TOLRPTAX", "INVPIND"
      ),
      c(
        "NAT_PIND", "CYPRPTAX", "RHEOPIND", "NAT_MIGRPTAX", "LITHNTAX", "NAT_NTOLNTAX", "TOLRNTAX",
        "HERBPTAX"
      ),
      c(
        "ALIENNTAX", "NAT_ICTAPIND", "RHEONTAX", "INTLMIGRNTAX", "LITHPIND", "NAT_NTOLNTAX", "INTLPTAX",
        "CARNNTAX"
      ),
      c(
        "NAT_PTAX", "CYPRNTAX", "INTLLOTNTAX", "INTLMIGRPTAX", "LITHPIND", "NTOLNTAX", "TOLRNTAX",
        "INTLINVPTAX"
      ),
      c(
        "NAT_PIND", "NAT_CATOPIND", "INTLLOTPTAX", "NAT_MIGRPTAX", "LITHPTAX", "NAT_TOTLNTAX",
        "TOLRNTAX", "NAT_HERBPTAX"
      ),
      c(
        "NAT_PIND", "CENTPTAX", "RHEOPIND", "MIGRPTAX", "LITHNTAX", "NTOLPTAX", "TOLRNTAX",
        "BENTINVPTAX"
      )
    ),
    stringsAsFactors = F
  ...
  varLong <- names(inMets)[names(inMets) %in% unique(metnames$PARAMETER)]

  matchMets <- reshape(inMets,
    idvar = c("SAMPID", "ECO9", "LWSAREA"), direction = "long",
  ...
  wsMets <- data.frame(
    ECO9 = c(
      rep("CPL", 5), rep("NAP", 3), rep("NPL", 6), rep("SAP", 3), rep("SPL", 2),
      rep("TPL", 4), rep("UMW", 3), rep("WMT", 3), rep("XER", 4)
    ),
    PARAMETER = c(
      c("ALIENPIND", "LOTPIND", "LITHPIND", "NAT_TOTLNTAX", "TOLRNTAX"),
    ...
    int = c(
      c(-0.219734, 83.680193, 90.591166, 10.929299, 1.831029),
    ...
    slope = c(
      c(0.178533, -5.644243, -21.2575, 2.873952, 1.559498),
  ...
  fish.ws <- merge(matchMets, wsMets, by = c("ECO9", "PARAMETER"), all.x = TRUE)
  ...
  cfVal <- data.frame(
    ...
    PARAMETER = c(
      c(
        "ALIENPIND_WS", "RBCATONTAX", "LOTPIND_WS", "INTLMIGRPTAX", "LITHPIND_WS", "NAT_TOTLNTAX_WS", "TOLRNTAX_WS",
        "INVPTAX"
      ),
      c(
        "ALIENNTAX", "SALMNTAX", "NAT_RHEOPIND", "INTLMIGRPIND", "LITHPTAX_WS", "NTOLPTAX_WS", "TOLRNTAX_WS",
        "INVNTAX"
      ),
      c(
        "ALIENNTAX", "NAT_CYPRPIND", "LOTNTAX_WS", "MIGRNTAX_WS", "LITHPIND_WS", "NTOLPTAX_WS",
        "NAT_INTLPIND_WS", "NAT_CARNNTAX_WS"
      ),
      c(
        "NAT_PTAX", "NAT_CENTNTAX_WS", "NAT_NTOLBENTPTAX", "NAT_MIGRNTAX", "NAT_LITHPIND_WS", "NTOLPTAX",
        "TOLRPTAX", "INVPIND_WS"
      ),
      c(
        "NAT_PIND", "CYPRPTAX_WS", "RHEOPIND", "NAT_MIGRPTAX_WS", "LITHNTAX", "NAT_NTOLNTAX", "TOLRNTAX",
        "HERBPTAX"
      ),
      c(
        "ALIENNTAX_WS", "NAT_ICTAPIND_WS", "RHEONTAX", "INTLMIGRNTAX", "LITHPIND", "NAT_NTOLNTAX_WS", "INTLPTAX",
        "CARNNTAX_WS"
      ),
      c(
        "NAT_PTAX", "CYPRNTAX", "INTLLOTNTAX_WS", "INTLMIGRPTAX", "LITHPIND", "NTOLNTAX_WS", "TOLRNTAX_WS",
        "INTLINVPTAX"
      ),
      c(
        "NAT_PIND", "NAT_CATOPIND", "INTLLOTPTAX_WS", "NAT_MIGRPTAX_WS", "LITHPTAX", "NAT_TOTLNTAX_WS",
        "TOLRNTAX", "NAT_HERBPTAX"
      ),
      c(
        "NAT_PIND", "CENTPTAX", "RHEOPIND", "MIGRPTAX_WS", "LITHNTAX_WS", "NTOLPTAX", "TOLRNTAX_WS",
        "BENTINVPTAX_WS"
      )
    ),
    FLOOR = c(
      c(-0.494, 0, -73.797, 0, -81.385, -15.491, -3.595, 9.09),
    ...
    CEILING = c(
      c(14.277, 3, 30.655, 5.88, 33.825, 7.213, 7.123, 68.75),
  ...
  mmi <- aggregate(
    x = list(RESULT = scored.mets$RESULT), by = scored.mets[c("SAMPID", "ECO9")],
    FUN = function(x) {
      round(sum(x) * (10 / 8), 2)
    }
  )
  ...
  dfOut.1.wide <- reshape(dfOut.1,
    idvar = c("SAMPID", "ECO9"), direction = "wide",
