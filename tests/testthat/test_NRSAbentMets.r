...
test_that("Data prep correct", {
  testOut <- prepBentCts_WSA(
    inCts = bentCts_test, inTaxa = bentTaxa_nrsa,
  ...
  testOut.long <- reshape(testOut,
    idvar = c("UID", "SAMPLE_TYPE", "SAMPLE_CAT", "TAXA_ID"), direction = "long",
...
test_that("Benthic Taxonomy metric values correct", {
  testOut <- calcBentTaxMets(indf_test,
    inTaxa = bentTaxa_nrsa, sampID = c("UID", "SAMPLE_TYPE", "SAMPLE_CAT"),
    dist = "IS_DISTINCT", ct = "TOTAL"
  )
  varLong <- names(testOut)[names(testOut) %nin% c("UID", "SAMPLE_TYPE", "SAMPLE_CAT")]
  testOut.long <- reshape(testOut,
    idvar = c("UID", "SAMPLE_TYPE", "SAMPLE_CAT"), direction = "long",
...
test_that("Benthic FFG metric values correct", {
  testOut <- calcBentFFGmets(indf_test,
    inTaxa = bentTaxa_nrsa, sampID = c("UID", "SAMPLE_TYPE", "SAMPLE_CAT"),
...
test_that("Benthic Habit metric values correct", {
  testOut <- calcBentHabitMets(indf_test,
    inTaxa = bentTaxa_nrsa, sampID = c("UID", "SAMPLE_TYPE", "SAMPLE_CAT"),
...
test_that("Benthic tolerance metric values correct", {
  testOut <- calcBentTolMets(indf_test,
    inTaxa = bentTaxa_nrsa, sampID = c("UID", "SAMPLE_TYPE", "SAMPLE_CAT"),
...
test_that("Benthic dominance and diversity metric values correct", {
  testOut <- calcBentDominMets(indf_test,
    inTaxa = bentTaxa_nrsa, sampID = c("UID", "SAMPLE_TYPE", "SAMPLE_CAT"),
    dist = "IS_DISTINCT", ct = "TOTAL"
  )
  varLong <- names(testOut)[names(testOut) %nin% c("UID", "SAMPLE_TYPE", "SAMPLE_CAT")]
...
test_that("All benthic metric values correct", {
  testOut <- calcAllBentMets(
    indf = indf_test, inTaxa = bentTaxa_nrsa, sampID = c("UID", "SAMPLE_TYPE", "SAMPLE_CAT"),
...
test_that("MMI metrics correct", {
  ecoTest <- data.frame(
    UID = c(11245, 11703, 11821, 12483, 12571, 12999, 13971, 14822, 15073, 15803),
    AGGR_ECO9_2015 = c(
      "TPL", "CPL", "NAP", "SAP", "SAP", "UMW", "CPL", "NPL",
      "UMW", "XER"
    ), stringsAsFactors = F
  )
  indf.eco <- merge(indf_test, ecoTest, by = "UID")

  testOut <- calcNRSA_BentMMImets(
    inCts = indf.eco, sampID = c("UID", "SAMPLE_TYPE", "SAMPLE_CAT"),
    dist = "IS_DISTINCT", ct = "TOTAL", taxa_id = "TAXA_ID", ffg = "FFG_WSA",
  ...
  testOut.long <- reshape(testOut,
    idvar = c("UID", "SAMPLE_TYPE", "SAMPLE_CAT", "AGGR_ECO9_2015"), direction = "long",
    varying = varLong, timevar = "PARAMETER",
    v.names = "RESULT", times = varLong
  )
...
test_that("NRSA Benthic MMI scores correct", {
  ...
  testIn.wide <- reshape(testIn,
    idvar = c("UID", "SAMPLE_TYPE", "SAMPLE_CAT", "AGGR_ECO9_2015"), direction = "wide",
    v.names = "RESULT", timevar = "PARAMETER"
  )
  names(testIn.wide) <- gsub("RESULT\\.", "", names(testIn.wide))

  testOut <- calcNRSA_BenthicMMI(testIn.wide,
    sampID = c("UID", "SAMPLE_TYPE", "SAMPLE_CAT"),
    ecoreg = "AGGR_ECO9_2015", totlnind = "TOTLNIND"
  )
  varLong <- names(testOut)[names(testOut) %nin% c("UID", "SAMPLE_TYPE", "SAMPLE_CAT")]
  testOut.long <- reshape(testOut,
    idvar = c("UID", "AGGR_ECO9_2015", "SAMPLE_TYPE", "SAMPLE_CAT"), direction = "long",
    varying = varLong, timevar = "PARAMETER",
    v.names = "RESULT", times = varLong
  )
  ...
  bentMMI_test.long <- reshape(bentMMI_test,
