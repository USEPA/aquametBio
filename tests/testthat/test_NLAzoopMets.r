...
test_that("Data prep correct", {
  testOut <- convertZoopCts_NLA(rawZoop_test,
    sampID = "UID",
...
test_that("Combine fine and coarse mesh samples correct", {
  testOut <- prepZoopCombCts_NLA(zpCts_test,
    sampID = "UID",
    sampType = "SAMPLE_TYPE",
...
test_that("MMI scores and condition correct", {
  ecoreg <- data.frame(
    UID = c(
      2010916, 2010606, 2010513, 2011008, 2010600,
      2011314, 2010571, 2011195, 2010632, 2011304
    ),
    ECO_BIO = c(
      "EHIGH", "EHIGH", "EHIGH", "UMW", "EHIGH",
  ...
  testOut <- calcNLA_ZoopMMI(mmiIn,
    sampID = "UID", ecoreg = "ECO_BIO",
    totlnind = "TOTL_NIND"
  )
  varLong <- names(testOut)[names(testOut) %nin% c("UID", "ECO_BIO")]
  testOut.long <- reshape(testOut,
    idvar = c("UID", "ECO_BIO"), direction = "long",
  ...
  zoopMMI_test.long <- reshape(zoopMMI_test,
