...
test_that("Fish tolerance metric values correct", {
  testOut <- calcFishTolMets(fishCts_test,
    sampID = c("UID"), dist = "IS_DISTINCT", ct = "FINAL_CT",
...
test_that("Fish taxonomic metric values correct", {
  testOut <- calcFishTaxMets(fishCts_test, fishTaxa,
    sampID = c("UID"), dist = "IS_DISTINCT", ct = "FINAL_CT",
...
test_that("Fish trophic metric values correct", {
  testOut <- calcFishTrophicMets(fishCts_test, fishTaxa,
    sampID = c("UID"), dist = "IS_DISTINCT", ct = "FINAL_CT",
...
test_that("Fish other metric values correct", {
  testOut <- calcFishOtherMets(fishCts_test, fishTaxa,
    sampID = c("UID"), dist = "IS_DISTINCT", ct = "FINAL_CT",
...
test_that("Fish native/alien metric values correct", {
  testOut <- calcFishNativeMets(fishCts_test,
    sampID = c("UID"), dist = "IS_DISTINCT", ct = "FINAL_CT",
...
test_that("Fish anomaly metric values correct", {
  testOut <- calcFishAnomMets(fishCts_test,
    sampID = c("UID"), ct = "FINAL_CT",
    anomct = "ANOM_CT"
  )
  varLong <- names(testOut)[names(testOut) %nin% c("UID")]
  testOut.long <- reshape(testOut,
...
test_that("All fish metric values correct", {
  testOut <- calcAllFishMets(fishCts_test, fishTaxa,
...
test_that("MMI fish metric values correct", {
  ...
  testOut.long <- reshape(testOut,
    idvar = c("UID", "AGGR_ECO9_2015"), direction = "long",
    varying = varLong, timevar = "PARAMETER",
    v.names = "RESULT", times = varLong
  )
...
test_that("Fish MMI scores correct", {
  ...
  fishws_test.1 <- reshape(fishws_test,
    idvar = c("UID", "SAMPLE_TYPE", "LWSAREA", "AGGR_ECO9_2015"), direction = "wide",
  ...
  testOut <- calcFishMMI(fishws_test.1,
    sampID = c("UID", "SAMPLE_TYPE"), ecoreg = "AGGR_ECO9_2015",
    lwsarea = "LWSAREA"
  )
  testOut.long <- reshape(testOut,
    idvar = c("UID", "AGGR_ECO9_2015"), direction = "long",
  ...
  fishMMI_test.long <- reshape(fishMMI_test,
    idvar = c("UID"), direction = "long",
...
test_that("Fish MMI condition correct", {
  ...
  testOut <- assignFishCondition(fishws_test.2,
    sampID = c("UID", "SAMPLE_TYPE"), ecoreg = "AGGR_ECO9_2015",
    wsarea = "WSAREA", totlnind = "TOTLNIND", mmi = "MMI_FISH"
  )
  testOut.1 <- testOut[, c("UID", "SAMPLE_TYPE", "MMI_FISH", "FISH_MMI_COND")]
  testOut.long <- reshape(testOut.1,
    idvar = c("UID", "SAMPLE_TYPE"), direction = "long",
