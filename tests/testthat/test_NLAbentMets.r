...
test_that("Benthic Taxonomy metric values correct", {
  testOut <- calcBentTaxMets(bentctsNLA_test,
    inTaxa = bentTaxa_nla, sampID = c("SITE_ID"),
    dist = "IS_DISTINCT", ct = "TOTAL"
  )
  varLong <- names(testOut)[names(testOut) %nin% c("SITE_ID")]
  testOut.long <- reshape(testOut,
    idvar = c("SITE_ID"), direction = "long",
...
test_that("Benthic FFG metric values correct", {
  testOut <- calcBentFFGmets(bentctsNLA_test,
    inTaxa = bentTaxa_nla, sampID = c("SITE_ID"),
...
test_that("Benthic Habit metric values correct", {
  testOut <- calcBentHabitMets(bentctsNLA_test,
    inTaxa = bentTaxa_nla, sampID = c("SITE_ID"),
...
test_that("Benthic tolerance metric values correct", {
  testOut <- calcBentTolMets(bentctsNLA_test,
    inTaxa = bentTaxa_nla, sampID = c("SITE_ID"),
...
test_that("Benthic dominance and diversity metric values correct", {
  testOut <- calcBentDominMets(bentctsNLA_test,
    inTaxa = bentTaxa_nla, sampID = c("SITE_ID"),
    dist = "IS_DISTINCT", ct = "TOTAL"
  )
  varLong <- names(testOut)[names(testOut) %nin% c("SITE_ID")]
...
test_that("All benthic metric values correct", {
  testOut <- calcAllBentMets(
    indf = bentctsNLA_test, inTaxa = bentTaxa_nla, sampID = c("SITE_ID"),
...
test_that("MMI metrics correct", {
  indf.eco <- subset(bentMetsNLA_test, select = c("SITE_ID", "ECO_BIO")) %>%
    unique() %>%
    merge(bentctsNLA_test, by = "SITE_ID")

  testOut <- calcNLA_BentMMImets(
    inCts = indf.eco, inTaxa = bentTaxa_nla, sampID = c("SITE_ID"),
    dist = "IS_DISTINCT", ct = "TOTAL", taxa_id = "TAXA_ID", ffg = "FFG",
...
test_that("NRSA Benthic MMI scores correct", {
  testIn <- reshape(bentMetsNLA_test,
    idvar = c("SITE_ID", "ECO_BIO"), direction = "wide",
    timevar = "PARAMETER", v.names = "RESULT"
  )
  names(testIn) <- gsub("RESULT\\.", "", names(testIn))

  testOut <- calcNLA_BenthicMMI(testIn,
    sampID = c("SITE_ID"),
  ...
  bentMMI_NLA_test.long <- reshape(bentMMI_NLA_test,
    idvar = "SITE_ID", direction = "long",
